use super::{cells::Grid, manipulation::{push, rotate_by}, direction::Direction, cell_data::{MOVER, GENERATOR, ROTATOR_CCW, ROTATOR_CW}};
    ...
    loop_each!(for x, y, cell in grid; {
        if !cell.updated() {
            if cell.id() == ROTATOR_CW {
                cell.set_updated(true);
                rotate_by(grid, x + 1, y, Direction::Down);
                rotate_by(grid, x, y - 1, Direction::Down);
                rotate_by(grid, x - 1, y, Direction::Down);
                rotate_by(grid, x, y + 1, Direction::Down);
            }
            else if cell.id() == ROTATOR_CCW {
                cell.set_updated(true);
                rotate_by(grid, x + 1, y, Direction::Up);
                rotate_by(grid, x, y - 1, Direction::Up);
                rotate_by(grid, x - 1, y, Direction::Up);
                rotate_by(grid, x, y + 1, Direction::Up);
            }
        }
